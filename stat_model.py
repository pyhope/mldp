...
inputs = {
    'model': args.model,
    'rand_seed': 0 if args.rand_seed is None else args.rand_seed,
    'system': None,
...
from dp_test import test_ener, train_ener, train_force_by_type
...
for path in paths:
    ...
    try:
        assert os.path.exists(os.path.join(path, args.deepmd))
    ...
    test_flag, train_flag = False, False
    train_force_type_results = []

    natoms = None
    sigma = None

    if os.path.exists(os.path.join(deepmd_path, 'set.001')):
        print('set.001 exist, test')
        test_flag = True
    ...
    if os.path.exists(os.path.join(deepmd_path, 'set.000')):
        print('set.000 exist, train')
        train_flag = True
        num_tr, sigma, natoms, l2e_tr, l2ea_tr, l2f_tr, l2v_tr = train_ener(inputs)

        # one single dp.eval for all atom types
        num_tr_force, sigma_force, train_force_type_results = train_force_by_type(inputs)

        # normally num_tr_force == num_tr, sigma_force == sigma
        # no need to overwrite unless sigma is still None
        if sigma is None:
            sigma = sigma_force
    ...
    with open(logfile, 'w') as log:
        ...
        if train_flag:
            ...
            log.write('\n')
            log.write("# number of train data : %d " % num_tr)
            log.write('\n')
            ...
            log.write('## train force error by atom type ##\n')
            for sel_type, natoms_type, l2f_type in train_force_type_results:
                log.write("# type {0}: natoms = {1}, force_l2err = {2:e} eV/A\n".format(
                    sel_type, natoms_type, l2f_type
                ))

        if test_flag:
            log.write('## test results stored at {0}.*.out##'.format(args.detail_file))
            log.write('\n')
